...
#class jalan, ini berfungsi untuk nyimpan lajur jalannya
class Lane:
...
def callback(data):
    ...
    cv2.imshow("oriimage",frame)
    cv2.waitKey(10)

    # kode untuk publisher
    # plt.imshow(frame)
    # plt.show()

if __name__ == "__main__":
    rospy.init_node("detect_garis" , anonymous=True)
    try:
        rospy.Subscriber("/catvehicle/camera_front/image_raw_front", Image, callback)
        #drive_pub = rospy.Publisher("/catvehicle/cmd_vel_safe", Twist, queue_size=10)

        rospy.spin()  # fungsi callback dijalankan saat mencapai rospy.spin() ini
